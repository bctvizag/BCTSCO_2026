...
export const ReportTable_1 = ({
  ...
  const totals = useMemo(() => {
    ...
    for (const col of columns) {
      if (numericColumns[col]) {
        t[col] = sortedData.reduce((acc, row) => acc + toNumber(row[col]), 0);
      } else {
        t[col] = "";
      }
    }
    return t;
  }, [columns, sortedData, numericColumns]);
  ...
  const renderFooterContent = (colDef, idx) => {
    ...
    if (!numericColumns[colDef.key]) return "";
